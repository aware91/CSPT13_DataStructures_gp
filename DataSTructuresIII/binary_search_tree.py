...
class BSTNode:
    """
        ...
        The right subtree of a node contains only nodes with values greater than or equal than the node’s values.
    ...
    def insert(self, value):
        # check if the new nodes value is less than current nodes values
        ...
        # otherwise (the new nodes values is greater than or equal to the current node value)
    ...
    def contains(self, target):        
        # if the value of the current node matches target
            # return True
        
        # check if the target is less than current nodes values
            # if there is no left child already here
                # return False
            # otherwise
                # return a call of contains on the Left child passing in the target value
        # otherwise (the target is greater than the current node value)
            # if there is no right child already here
                # return False
            # otherwise
    ...
    def get_max(self):  ##it is the while loop in get_max in the doubly link list
        # check for an empty tree
            # return None
        # recursive approach
        # check if there is no node to the right
            # return the nodes value
        # return a call to get max on the right
        """
            # iterative approach
    ...
    def for_each(self, fn):
        ...
        # if there is anode on the right
